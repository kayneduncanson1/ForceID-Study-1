...
from Models import FCNN, CNN, CLSTMNN, CTNN, CBiLSTMNN
from Losses import BatchHardTripletSuppPt2
from TrainEval import train_val_supp_pt2, test_supp_pt2
from Utils import load_channel_pro, split_trial_names_by_ID_and_sess, generate_cv_folds

"""This script shows how to run the experiment for the 'All' condition in Supp Material - Section I. Please see
'..._supp_pt2' funcs within the TrainEval package and the 'BatchHardTripletSuppPt2' class within the Losses package
for further details."""
...
m = 0.3
criterion = BatchHardTripletSuppPt2(margin=m)
epochs = 1000
interp_len = 300

# The benchmark set-up from the main experiments was applied on D_AS_AF for this experiment:
datasets = ['D_AS_AF']
archs = ['FCNN']
batch_sizes = [512]
input_channels = ['All']
...
for setting in expt_settings:
    ...
    for fold in range(7):
        ...
        # Note that val and test sets below aren't constrained to contain min_sample_counts as in main.py:
        trial_names_va = np.concatenate([np.concatenate((trial_names_s1_va[fold][i], trial_names_s2_va[fold][i]))
                                         for i in range(trial_names_s2_va[fold].shape[0])])

        trial_names_te = np.concatenate([np.concatenate((trial_names_s1_te[fold][i], trial_names_s2_te[fold][i]))
                                         for i in range(trial_names_s1_te[fold].shape[0])])

        # Define where to get samples from each session within val and test sets:
        indices_s1_va = [idx for idx, name in enumerate(trial_names_va) if name[4] == '1']
        indices_s2_va = [idx for idx, name in enumerate(trial_names_va) if name[4] == '2']

        indices_s1_te = [idx for idx, name in enumerate(trial_names_te) if name[4] == '1']
        indices_s2_te = [idx for idx, name in enumerate(trial_names_te) if name[4] == '2']
        ...
        time_tr_va, accs_tr, losses_tr, accs_va, losses_va, mod_state_dict, opt_state_dict, embs_va =\
            train_val_supp_pt2(mod, data_loader_tr, data_loader_va, labels_va, criterion, indices_s1_va, indices_s2_va,
                               opt, epochs)
        ...
        loss_te, acc_te, embs_te = test_supp_pt2(mod, opt, data_loader_te, labels_te, criterion,
                                                 indices_s1_te, indices_s2_te)
